...
int sockfd;
int led_flag;
pthread_t client,blink2,blink3;
int m,n;

void sig_blink(int sig)
{
...
void *thread_fun(void *arg)
{
	...
	while(1)
	{
		...
		if((s[0]-'R')==0 ){
			m=1;
			n=0;
			pthread_kill(blink2,SIGUSR1);}
		if((s[0]-'O')==0)
		{
			n=1;
			m=0;
			pthread_kill(blink3,SIGUSR2);
...
int main()
{
	...
	int ret2 = pthread_create(&blink3,NULL,thread_fun3,(void *)NULL);
	if (ret2 < 0){
		perror("pthread create fail:");
		exit(0);
	}
