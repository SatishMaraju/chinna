...
pthread_t server,client,blink,stop;

void blink_hand(int sig);
void stop_hand(int sig);
...
{
	signal(SIGUSR1,blink_hand);
	//signal(SIGUSR2,stop_hand);
	while(1)
	{
		pause();
	}
...
void *thread_fun4(void *arg)
{
	...
	while(1)
	{
		pause();//wait for the signal and decides to process or to terminate
	}
}
...
int main()
{
	...
	int ret1 = pthread_create(&blink,NULL,thread_fun3,(void *)NULL);
	if (ret1 < 0){
		perror("pthread create fail:");
		exit(0);
	}
	int ret2 = pthread_create(&stop,NULL,thread_fun4,(void *)NULL);
	if (ret2 < 0){
		perror("pthread create fail:");
		exit(0);
	}
	...
	pthread_join(blink,&retval);
	pthread_join(stop,&retval);
